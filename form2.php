...
<div id="mydiv8"><br>Запросы <button value="Очистить" onclick="ochist()"> Очистить</button></div>
<div id="mydiv5"></div>
<script>
...
function selection(name,what,id,st){
	...
	$.ajax({ 
		...
		st2=what+' '+'<select id="textfield'+id.toString()+'_'+name+'">'
...
function showqueries(arr,k,mode){
	...
	for (l=0;l<arr.length;l++){
	   ...
	   if (flag==0){
		   if (mode==2){
		   if (arr[l].indexOf('(') + 1){
			st=st+arr[l]+'<br><br><input maxlength="4" size="4" style="visibility:visible" id="t_1'+l.toString()+'"> '+
			'<select id="character'+l.toString()+'" onchange="kb('+l+')"><option>=</option><option>>=</option>'+
			'option value=">"></option>option value="=<"></option><option value="<"></option><option value="not =">'+
			'</option></select> <input style="visibility:visible" maxlength="4" size="4" id="t_2'+l.toString()+'"><br><br>'
			} else {
...
function newtable(k){
 ...
 $.ajax ({
	...
	document.getElementById('mydiv8').innerHTML='<br>Запросы <button onclick="ochist()">Очистить</button><button onclick="ok('+k.toString()+')">Ok</button><button onclick="newtable('+k.toString()+')">Назад</button>'
}
...
var arr1=['Наименование','Цена(сум)','Производитель','Категория товара','Отдел','Срок хранения (суток)','Количество(штук)'] //greeds
 ...
 var mystr=''
		...
		for (i=0;i<arr.length;i++){
			if (ans[0]=="greeds" && (arr[i]=='Категория товара' || arr[i]=='Отдел')){
					continue
			}
			
			if (ans[0]=="postavshik" && (arr[i]=='Категория товара')){
					continue
			}
			
			if (ans[0]=="salesman" && (arr[i]=='ФИО' || arr[i]=='Отдел')){
					continue
			}
			...
			if (ans[0]=="sales" && (arr[i]=='Продавец' || arr[i]=='Товар')){
					continue
			}
...
function ok(m){
	...
	if (names.length==1){
		if (names[0]=="greeds g"){
			...
			"LEFT JOIN sklad skl ON g.`Код категории`=skl.`Номер` WHERE ";
		}
		
		if (names[0]=="personal pl"){
			var st="SELECT * FROM ("+names[0]+") WHERE "
		}
		
		if (names[0]=="postavshik pt"){
			var st="SELECT pt.`Поставщик`,pt.`Дата следующей поставки`,skl.`Категория товара`,pt.`Адрес`,pt.`Телефон` "+
			"FROM (postavshik pt LEFT JOIN sklad skl ON pt.`Код категории`=skl.`Номер`) WHERE "
		}
		...
		if (names[0]=="salesman sl"){
				...
				"LEFT JOIN personal pl ON sl.`Код сотрудника`=pl.`Номер` WHERE "
		}
		
		if (names[0]=="sklad skl"){
			var st="SELECT skl.`Категория товара`,skl.`Дата получения`,skl.`Номер стеллажа`,skl.`Номер полки`,pl.`ФИО` "+
			"FROM (sklad skl LEFT JOIN personal pl ON skl.`Получил`=pl.`Номер`) WHERE "
		}
		
		if (names[0]=="sales s"){
				var st="SELECT s.`Дата продажи`,s.`Продажа`,s.`Количество(штук)`,pl.`ФИО`,g.`Наименование`,s.`Скидка (процент)` "+
				"FROM (sales s LEFT JOIN greeds g ON s.`Код товара`=g.`Номер`) LEFT "+
				"JOIN salesman sl ON sl.`Номер`=s.`Код продавца` LEFT JOIN personal pl ON sl.`Код сотрудника`=pl.`Номер` WHERE "
		}
	}
	...
	if (names.length==2){
		...
		if (names[0]=="greeds g" && names[1]=="sales s"){
		  st=st+ "s.`Код товара`=g.`Номер`) LEFT JOIN otdel o ON g.`Код отдела`=o.`Номер` LEFT JOIN sklad skl ON g.`Код категории`=skl.`Номер` LEFT JOIN salesman sl ON s.`Код продавца`=sl.`Номер` LEFT JOIN personal pl ON pl.`Номер`=sl.`Код сотрудника`  WHERE "	
		}
		
		if (names[1]=="salesman sl" && names[0]=="personal pl"){
		  st=st+ "sl.`Код сотрудника`=pl.`Номер`) LEFT JOIN otdel o ON sl.`Код отдела`=o.`Номер` WHERE "	
		}
		
		if (names[1]=="sklad skl" && names[0]=="greeds g"){
		  st=st+ "g.`Код категории`=skl.`Номер`) LEFT JOIN otdel o ON g.`Код отдела`=o.`Номер` WHERE "	
		}
		
		if (names[0]=="postavshik pt" && names[1]=="sklad skl"){
		  st=st+ "pt.`Код категории`=skl.`Номер`) WHERE "	
		}
	}
	...
	if (names.length==3){
		if (names[0]=="greeds g" && names[2]=="sklad skl" && names[1]=="postavshik pt"){
		  ...
		  st=st+ "g.`Код категории`=skl.`Номер`) LEFT JOIN otdel o ON g.`Код отдела`=o.`Номер` LEFT JOIN postavshik pt ON pt.`Код категории`=skl.`Номер` WHERE "	
		}
		...
		if (names[0]=="personal pl" && names[1]=="salesman sl" && names[2]=="sales s"){
		     var st="SELECT * FROM ("+names[0]+" LEFT JOIN "+names[1]+" ON "
			 st=st+ "sl.`Код сотрудника`=pl.`Номер`) LEFT JOIN otdel o ON sl.`Код отдела`=o.`Номер`  LEFT JOIN sales s ON s.`Код продавца`=sl.`Номер` WHERE "	
		}
		
		if (names[0]=="personal pl" && names[1]=="postavshik pt" && names[2]=="sklad skl"){
			 var st="SELECT * FROM ("+names[2]+" LEFT JOIN "+names[1]+" ON "
			 st=st+ "pt.`Код категории`=skl.`Номер`) LEFT JOIN personal pl ON skl.`Получил`=pl.`Номер` WHERE "
		}
		
		if (names[0]=="personal pl" && names[1]=="salesman sl" && names[2]=="sklad skl"){
			 var st="SELECT * FROM ("+names[0]+" LEFT JOIN "+names[1]+" ON "
			 st=st+ "sl.`Код сотрудника`=pl.`Номер`) LEFT JOIN otdel o ON sl.`Код отдела`=o.`Номер` LEFT JOIN sklad skl ON skl.`Получил`=pl.`Номер` WHERE "
		}
	}
...
function ochist(){
  document.getElementById('mydiv5').innerHTML='';
  for (i=0;i<6;i++){
	 document.getElementById("q_"+i.toString()).checked=false 
  }
...
function give_search(arr,names){
	...
	for (t=0;t<arr.length;t++){
		for (l=0;l<arr[t].length;l++){
			if ((names[t]=="greeds" && l==0) || (names[t]=="personal" && l!=2 && l!=1 && l!=3 && l!=4) ||
				(names[t]=="postavshik" && l!=0 && l!=2) ||
				(names[t]=="salesman" && l==2) ||
				(names[t]=="sklad" && l!=0 && l!=4) || (names[t]=="sales" && l!=3 && l!=4)
			){
			...
			if (first==1 && p.length!=""){
				...
				if (names[t]=="greeds" && l==1){
					...
					if (znak==">="){
						...
						mystr=mystr+arr[t][l]
						arr_u.push(arr[t][l]+">"+"'"+a+"' AND "+arr[t][l]+"<="+"'"+b+"' AND ")
					}
